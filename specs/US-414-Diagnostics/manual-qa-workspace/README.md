...
| `MissingBrace.st` | dynamic array missing `}` | `Expected "}"` (L14) | `}` | `parse error, expected '}'` (L14) |
| `MissingAttribute.st` | method pragma missing `>` | `Expected ">" to close attribute` (L16) | `>` | `parse error, expected '>'` (L15) |
...
| 15 | `MissingBrace.st` | caret on the squiggle → `Ctrl/Cmd+.` | `Insert missing "}"`; applies and clears |
| 16 | `MissingAttribute.st` | caret on the squiggle → `Ctrl/Cmd+.` | `Insert missing ">"`; applies and clears |
