...
- **Next:** **0.6.0 / US-414** — diagnostics (parser live; `gst` opt-in). **EPIC-005 foundation
  underway** (lands 0.8/0.9): the Dialect Cartridge schema (`server/src/types/knowledge-base.ts`) + GST
  **Cartridge #01** (`scripts/export-gst-cartridge.st` → `server/data/cartridges/gst-3.2.5-cartridge.json`,
  249 classes / 4746 signatures, validated) are built; remaining is the Console loader + the
  kernel-index **convergence** (US-430).
...
- **Knowledge Graph / Cartridges (EPIC-005, US-430):** the dialect-neutral **Dialect Cartridge** schema
  lives in `server/src/types/knowledge-base.ts` (pure JSON, facts-only, class-side/instance-side split,
  superclass/traits resolution separate from an open `taxonomy` bag). GST **Cartridge #01** is generated
