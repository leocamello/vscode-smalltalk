...
- **Shipped:** **v0.11.0 — scope-aware rename (US-426, EPIC-005)** — `textDocument/rename` +
  ...
  **arguments**, and **instance variables**; selectors + classes are **rejected with a reason** (dynamic
  dispatch makes selector rename unsafe offline → Live Bridge; class rename split to **US-428/#109**).
  temps/args rewrite within the enclosing block/method; an **ivar renames workspace-wide** across every file
  that defines/`extend`s the class, **skipping any method that shadows the name with a local**. New-name
  **validation** refuses invalid identifiers + collisions/shadows (no cross-symbol bleed). **Multi-file
  renames force the Refactor Preview** (`needsConfirmation`, `withMultiFileConfirmation`, gated on client
  change-annotation support); single-file stays instant. Shared **shadow-aware** scope walk extracted to
  `server/src/parser/scope.ts` (reused by `documentHighlight`); provider `server/src/providers/rename.ts`;
  workspace-wide ivar resolver `server/src/xref/ivarRefs.ts`. New `evals/datasets/rename/` +
  `specs/US-426-Scope-Aware-Rename/manual-qa-workspace/`. Closes #69.
...
- **Source:** parser/symbols in `server/src/parser/` (`lexer.ts`, `parser.ts`, `ast.ts`, `symbols.ts`,
  `walk.ts`, **`scope.ts`** shared shadow-aware variable-scope walk; GST containers/chunk live in
  `parser.ts`). LSP providers in `server/src/providers/`
