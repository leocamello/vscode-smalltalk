...
> **Status summary (2026-07-06).** v0.2.0–**v1.0.0** are shipped — **the 1.0 offline-parity milestone is reached.** **Done:** US-101–106 & US-200–203 (declarative foundation, v0.2.0), US-301 (Run Current File, v0.3.0), US-410 (LSP scaffold, v0.3.0), **US-411** (error-tolerant parser + symbol table, internal milestone M3), **US-412** (outline + workspace symbols + go-to-definition, v0.4.0), **US-417** (semantic folding + scope-aware document highlight, v0.4.1), and **US-413** (completion + GNU Smalltalk kernel index, **v0.5.0** — closes #1), and **US-414** (diagnostics — **parser-only**: live squiggles + insert-closer/close-string quick fixes; the opt-in `gst` compile tier was deferred to EPIC-007 / **US-705**, **v0.6.0**), and **US-415** (hover — selectors/classes/variables/literals + provenance-gated comment prose; radix-integer coloring fix, **v0.7.0** — closes #27), and **US-422** (cartridge-aware semantic tokens — role-accurate highlighting + the offline known-class vs unknown-global distinction; the first user-facing cartridge consumer, **v0.8.0** — closes #65), and **US-423** (references + senders/implementors + call hierarchy over the two-tier engine; honest lexical union; installed-tier parity + real-source navigation, **v0.9.0** — closes #66), and **US-425** (keyword-message signature help over the two-tier index; active-parameter tracking; honest prefix union; **v0.9.1** — closes #68), and **US-427** (selector-surface coverage audit — ADR-0004 division of labour + 14 block-bearing snippets + cartridge-cross-check guard, **v0.9.2** — closes #102), and **US-416** (formatting — document/range/on-type over a whitespace-only token-stream rewriter, off by default, conservative + idempotent, **v0.10.0** — closes #28), and **US-426** (scope-aware rename — temporaries/arguments/instance variables; workspace-wide ivar rename with multi-file Refactor-Preview confirmation; selectors/classes rejected-with-reason; **v0.11.0** — closes #69), and **US-428** (class rename — workspace-wide across every reference form incl. namespaced `#{…}`/`A.B`/`A::B`, resolution-gated; kernel-boundary-safe reject-with-reason; multi-file Refactor Preview; **v0.12.0** — closes #109), and **US-901** (hardening & perf — parser never-throws with a depth cap + `RangeError` net → one diagnostic; committed perf bench proving 1k files ≈0.2 s / completion p95 ≈5 ms; no-telemetry guard; fuzz + 735-check provider matrix; **v0.13.0** — closes #112), and **US-902** (product polish & Open VSX — formatting graduated to always-on = removed `smalltalk.format.enable`; removed `preview`; settings reorder (GST path first); Workspace-Trust declared + Run-refused-untrusted; virtual-workspace declared; cancellation-token plumbing; published to Open VSX in CI; README demo + shot-list + issue templates + labels; **v1.0.0** — the 1.0 milestone; incorporates the two US-901 deferrals). **Next (post-1.0):** the **read-only Tonel wedge** (US-424, EPIC-006) and the **Image-Grade Workbench** (EPIC-008 — System Browser view, class-hierarchy view, extract-method); the optional **Live Bridge** (EPIC-007) stays post-1.x. **SPIKE-01** (unknown-selector heuristic) is **done — shelved** (#67; zero-FP bar unmet + low closed-world coverage; gate parked, reconsider after cartridge completeness). **Backlog (0.5.0 plan reconciliation):** US-418 (dialect seam, deferred), US-419 (kernel categories), US-420 (completion pseudo-variables), US-421 (CI kernel fixtures). US-901 (hardening/perf) **shipped as 0.13.0**; US-902 (1.0.0 polish + Open VSX) **shipped as 1.0.0**. **Superseded by [ADR-0001](../decisions/0001-typescript-bundled-lsp-server.md):** US-401–403 (the server is TypeScript, not Smalltalk). The per-story `Status` fields below reflect this; see [`docs/ROADMAP.md`](../ROADMAP.md) for the live milestone view. **EPIC-005 (Offline Knowledge Graph / "Console & Cartridges")** opens the next arc: US-430 (cartridge schema + GST Cartridge #01 reflective exporter), US-422 (cartridge-aware semantic tokens), US-423 (references/senders/implementors, two-tier engine), US-425 (signature help, v0.9.1), SPIKE-01 (unknown-selector heuristic — **shelved**), and US-431 (kernel-sourcing transparency — installed version label + settings UX, deferred from US-430). The post-1.0 vision continues in **EPIC-006** (multi-dialect — US-601–603 + US-418, plus **US-424 read-only Tonel — the "Trojan Horse" wedge pulled forward to ~1.0**), **EPIC-007** (optional Live Bridge — US-701–706, incl. **US-705 runtime compile/semantic diagnostics deferred from US-414** and **US-706 writable `smalltalk-image://` VFS**) and **EPIC-008** (image-grade workbench — US-801–804; US-804 extract-method builds on the US-426 rename engine). Three ideas from a 2026-06-24 strategy review are folded in: US-424 (Tonel-first), US-706 (live-image VFS), and a status-bar dialect picker on US-602.
...
* **ID:** US-416
* **Status:** Done (v0.10.0 — formatting; document/range/on-type over a whitespace-only token-stream rewriter, off by default; closes #28)
* **Epic:** EPIC-004
...
* **ID:** US-901
* **Status:** ✅ Shipped — **v0.13.0** (#112, merged via #113). See `specs/US-901-Hardening-Perf/` for the delivered spec + verification (manual-QA passed; caught + fixed a diagnostic-flood defect).
...
**Definition of Done (DoD) Checklist:**
* [X] Perf budgets met + measured (`npm run bench`: 1k files ≈0.2 s, completion p95 ≈5 ms); robustness hardened (never-throw fuzz + provider matrix); no-telemetry verified (guard test). *Untrusted-/virtual-workspace capability declarations deferred to US-902.*
* [X] PO accepts (manual-QA passed; merged #113 → v0.13.0).
...
* **ID:** US-902
* **Status:** Done (v1.0.0) — the 1.0 offline-parity milestone. Detailed source of truth: `specs/US-902-Product-Polish-Open-VSX/`.
* **Epic:** EPIC-004 (cross-cutting)
* **Priority:** Medium
* **Estimate:** M
* **Date Proposed:** 2026-06-21
...
**User Story:**
> As a **user**, I want a **polished 1.0 with discoverable docs, honest workspace-trust behaviour, and availability beyond the VS Code Marketplace**, so that **the extension reads as a finished, trustworthy product.**

**Acceptance Criteria (AC)** *(the spec expanded the original AC1–4 to AC1–9 during Clarify; original scope preserved, plus the two US-901 deferrals + two owner directives)*:
* AC1: `"preview": true` removed from `package.json`.
* AC2: **Formatting graduated to always-on** — `smalltalk.format.enable` removed (whitespace-only + idempotent; runs on the editor's format gestures). *(Supersedes US-416 AC4.)*
* AC3: **Settings reordered** so `smalltalk.gnuSmalltalkPath` is the first setting (explicit `order`).
* AC4: **Untrusted-workspace** capability declared (`limited` + `restrictedConfigurations: [smalltalk.gnuSmalltalkPath]`); static intelligence works in Restricted Mode; **Run Current File refused untrusted**.
* AC5: **Virtual-workspace** capability declared (`limited` + description); on-disk features degrade cleanly.
* AC6: **Cancellation-token plumbing** — workspace symbols / references / call hierarchy / rename honour the LSP token.
* AC7: **Published to Open VSX** (`ovsx` + `OVSX_PAT`) alongside `vsce` in CI.
* AC8: Product-polish docs — README **Demo** section + `docs/media-shotlist.md`; CONTRIBUTING linked; `.github/ISSUE_TEMPLATE/` + labels (`area:parser`, `area:lsp`, `good-first-issue`).
* AC9: `docs/product/*` + ROADMAP/README/CLAUDE/CHANGELOG synced to shipped reality; version bumped to 1.0.0.
...
**Definition of Done (DoD) Checklist:**
* [X] 1.0 polish complete; Open VSX publish wired in CI (inert until `OVSX_PAT` provisioned); all test/eval layers green (manifest + cancellation + trustGate units, US-902 e2e); doc-rot sweep done; version 1.0.0.
* [ ] PO accepts. *(Owner handoffs before the `v1.0.0` tag: record the demo GIFs per `docs/media-shotlist.md`; provision the `OVSX_PAT` secret; create the release.)*

**Notes / Questions / Assumptions:**
* Open VSX needs an `OVSX_PAT` secret (owner-provisioned), analogous to `MARKETPLACE`. The CI step skips cleanly when it's absent.
* GIF assets can't be recorded in the build environment — the README carries the structure + captions and a deterministic shot-list; the owner records and drops `media/demo-*.gif` before tagging.
...
**Notes / Questions / Assumptions:**
* Parity-plus filler graded honestly; small. Keyword-only by design (unary/binary carry no parameter sequence). Honest prefix union with provenance (the US-423 posture); the active parameter is the keyword being filled; the highlighted parameter is the **keyword part** (facts-only cartridge has no arg names). Built on the spec package `specs/US-425-Signature-Help/`; shipped **v0.9.1**. The completion-vs-signature-help double-popup is standard VS Code behaviour; the division of labour is being formalised in **US-427**.
...
* **ID:** US-426
* **Status:** Done (v0.11.0 — scope-aware rename of temps/args/instance variables; workspace-wide ivar rename with multi-file Refactor-Preview confirmation; selectors/classes rejected-with-reason; closes #69). Class rename split out to US-428 (#109).
* **Epic:** EPIC-005
* **Priority:** Medium
* **Estimate:** M
* **Date Proposed:** 2026-06-22
...
* **ID:** US-427
* **Status:** Done (**v0.9.2** — selector-surface audit: ADR-0004 + 14 block snippets + guard; manual-QA passed 2026-06-27) — closes #102 *(raised during US-425 / v0.9.1 manual QA)*
* **Epic:** EPIC-005
* **Priority:** Low
* **Estimate:** S
...
**Definition of Done (DoD) Checklist:**
* [x] Inventory ([`spec.md §5.1`](../../specs/US-427-Selector-Surface-Coverage-Audit/spec.md)) +
  division-of-labour doc ([ADR-0004](../decisions/0004-selector-surface-division.md)); expanded
  `snippets.json` (14 block idioms) with eval guard (`src/test/snippets-verification.js`). *(PO sign-off
  + Extension-Host manual QA pending.)*
...
* **Delivered:** ADR-0004 pins the contract; the guard cross-checks snippet selectors against
  Cartridge #01 — which corrected two backlog names (`valuesDo:` dropped — not a GST selector;
  `withIndexDo:` → the real `doWithIndex:`).
