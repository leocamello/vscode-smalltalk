...
**Delta (2026-07-01).** **Scope-aware Rename (US-426) shipped as 0.11.0**, also pulled ahead of 1.0 (temps/
args/ivars only — selector rename is unsafe offline and stays in the Live Bridge, EPIC-007; **class rename**
...
**Delta (2026-07-06).** **Hardening & Perf (US-901) shipped as 0.13.0** — a beta-quality pass, no new
feature surface: the parser now **never throws** on pathological nesting (depth cap + top-level `RangeError`
net, degrading to a single diagnostic — a naïve high cap still overflowed the smaller macOS/Windows CI
stacks), a committed **perf bench** (`npm run bench` / synthetic corpus) proves the budgets with ~20×
headroom, a **no-telemetry guard** locks in the offline stance, and a fuzz + 735-check provider matrix guards
...
2. **EPIC-005 foundation has landed** (US-430, merged #82) ahead of its 0.8/0.9 milestones: the Dialect
   ...
   help) shipped v0.9.1**; **US-427 (selector-surface coverage audit — ADR-0004 + 14 block snippets +
   guard) shipped v0.9.2**. **SPIKE-01** (unknown-selector heuristic) was built + measured and **shelved**
   (zero-FP bar unmet; gate parked). Next: **US-416** (formatting, → ~1.0).
...
_Last updated: 2026-06-27 — **0.9.2 / US-427 (selector-surface coverage audit) shipped** (ADR-0004 division of labour + 14 block snippets + cartridge-cross-check guard; closes #102); **0.9.1 / US-425 (keyword-message signature help) shipped** (closes #68); **SPIKE-01 (unknown-selector heuristic) done — shelved** (#67: zero-FP bar unmet + low closed-world coverage; gate parked, reconsider after cartridge completeness — it did surface a real kernel typo, `primtiveFailed`, reported upstream). Earlier: **0.6.0 / US-414 (diagnostics) implemented, parser-only**: always-on parser
...
US-425 signature help shipped (v0.9.1); US-427 selector-surface audit shipped (v0.9.2)** — the offline knowledge graph's most visible navigation wins.
SPIKE-01 (unknown-selector heuristic) shelved. Next: US-416 (formatting, → ~1.0). **2026-06-24 strategy review folded in:**
